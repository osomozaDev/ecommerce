...
test("flujo de compra completo", async ({ page }) => {
  ...
  // Añadir al carrito → el indicador del header refleja 1
  await page.getByRole("button", { name: "Añadir al carrito" }).click();
  await expect(page.getByRole("banner").getByRole("link", { name: /Carrito/ })).toContainText("1");

  // Página de carrito: línea correcta
  await page.getByRole("banner").getByRole("link", { name: /Carrito/ }).click();
  ...
  await expect(page.getByRole("banner").getByRole("link", { name: /Carrito/ })).toContainText("2");
